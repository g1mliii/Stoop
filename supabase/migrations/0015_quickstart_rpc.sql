-- Phase 3.2: atomic seller + store + first product creation for quick-start signup.
--
-- Server-only RPC called by the auth callback after it verifies the signed quick-start cookie.
-- The whole function runs in one transaction, so a failure anywhere rolls back cleanly and
-- leaves no orphan rows (the Phase 3 "partial signup leaves no orphan rows" regression).
--
-- System defaults (visibility = 'qr_only', accept_pay_at_pickup = true, no category /
-- description / logo / building, Stripe not connected) come from the column defaults in
-- 0002_initial.sql — we deliberately do not set them here so the defaults stay single-source.
