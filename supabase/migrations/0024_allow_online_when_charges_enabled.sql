-- Phase 5.5: allow online orders once the seller's Stripe Connect account can take charges.
--
-- 0020-0023 categorically rejected payment_mode = 'online' (Phase 4 had no Stripe Checkout
-- handoff). Now that placeOrder creates a Checkout Session for online orders, both guards that
-- reject 'online' must instead permit it exactly when the store's seller has a connected_accounts
-- row with charges_enabled = true:
--   1. the place_order RPC's internal payment-mode check (raised before the insert), and
--   2. the orders_enforce_payment_mode trigger (the SQL backstop on every insert/update).
-- Both are updated here. Forward-only: each `create or replace` reproduces the whole body in
-- place without rewriting migration history (same pattern as 0021/0022).

-- 1. place_order RPC — reproduced verbatim from 0022 except the payment-mode guard (step 2).
...
-- 2. orders_enforce_payment_mode trigger — same online/pay-at-pickup logic as the RPC, so a
-- service-role insert can never create an online order for a not-yet-onboarded seller.
...
begin
  ...
  elsif new.payment_mode = 'pay_at_pickup'
    and not coalesce(v_accept_pay_at_pickup, false)
  then
    raise exception 'payment_mode_unavailable' using errcode = 'STP06';
  end if;
